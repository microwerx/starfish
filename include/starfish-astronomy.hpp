#ifndef STARFISH_ASTRONOMY_HPP
#define STARFISH_ASTRONOMY_HPP
...
namespace Sf {
	...
	struct Vector {
		...
		Vector(Real mu, Real nu) {
			Real mu_ = mu * SF_DEGREES_TO_RADIANS;
			Real nu_ = nu * SF_DEGREES_TO_RADIANS;
		...
		void toAngles(Real& mu, Real& nu) const {
			mu = atan2(y, x) * SF_RADIANS_TO_DEGREES;
			nu = asin(z) * SF_RADIANS_TO_DEGREES;
		}
