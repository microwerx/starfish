...
namespace Sf {
	...
	namespace PA {
		...
		Real TrueAnomaly(Real AM, Real EC) {
			Real M = AM - SF_TWOPI * trunc(AM / SF_TWOPI);
			Real AE = M;
		...
		Real SunLong(Real DJ, Real UT) {
			...
			Real AM = M1 * SF_DEGREES_TO_RADIANS;
			Real AT = TrueAnomaly(AM, EC);
			//FIXME: Real AE = EccentricAnomaly(AM, EC);

			A = 62.55209472 * T;
			B = 360.0 * (A - trunc(A));
			Real A1 = (153.23 + B) * SF_DEGREES_TO_RADIANS;
			A = 125.1041894 * T;
			B = 360.0 + (A - trunc(A));
			Real B1 = (216.57 + B) * SF_DEGREES_TO_RADIANS;
			A = 91.56766028 * T;
			B = 360.0 * (A - trunc(A));
			Real C1 = (312.69 + B) * SF_DEGREES_TO_RADIANS;
			A = 2136.853095 * T;
			B = 360.0 * (A - trunc(A));
			Real D1 = (350.74 - 0.00144 * T2 + B) * SF_DEGREES_TO_RADIANS;
			Real E1 = (231.19 + 20.2 * T) * SF_DEGREES_TO_RADIANS;
			A = 183.1353208 * T;
			B = 360.0 * (A - trunc(A));
			Real H1 = (353.4 + B) * SF_DEGREES_TO_RADIANS;
			...
			Real SR = AT + (L - M1 + D2) * SF_DEGREES_TO_RADIANS;
			SR = SR - SF_TWOPI * trunc(SR / SF_TWOPI);
			return SR * SF_RADIANS_TO_DEGREES;
		}
	...
	void AstroCalc::computeMatrices() {
		...
		Real phi = paTime.geo_latitude * SF_DEGREES_TO_RADIANS;
		Real ST = LST * 15.0 * SF_DEGREES_TO_RADIANS;
		Real e = obliquity_of_the_ecliptic * SF_DEGREES_TO_RADIANS;
	...
	EquatorialCoord AstroCalc::getMoon() const {
		...
		Real earthMoonDistance = 6378.14 / std::sin(SF_DEGREES_TO_RADIANS * moonHorzParallax);
